...
// things to do 1. when link to the comment Component, selected time should be passed down 
    return (
        <>
            <div className="w-1/2  bg-gray-300 ml-20 rounded-lg p-20">
                <h2 className="mb-10"><span className="text-blue-700">Here are available slots for </span> <span className="border border-blue-300 p-10 bg-white rounded-lg">{selectedDate.toDateString()}</span></h2>
                {appointments.map(appointment => {
                    ...
                    return (<ul className="bg-white rounded-lg mb-4 p-8 flex items-center justify-center">
                        <li className="pr-1">
                            ...
                            {appointment.personID == null && <Link className="p-8 bg-gray-300 rounded-lg ml-4 hover:bg-blue-300" to={`/calendar/`}>Select</Link>}
                        </li>
                    </ul>)
                })}
